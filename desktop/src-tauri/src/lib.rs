    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            std::thread::spawn(move || {
                ...
                if !server_up {
                    let error_html = r#"data:text/html,<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}:root{--fg:%23e1e4e8;--sub:%238b949e;--btn:%23238636;--btn-h:%232ea043}@media(prefers-color-scheme:light){:root{--fg:%231f2328;--sub:%23656d76;--btn:%231a7f37;--btn-h:%232da44e}}body{font-family:-apple-system,sans-serif;background:transparent;display:flex;align-items:center;justify-content:center;height:100vh;text-align:center;color:var(--fg)}h2{margin-bottom:.4rem;font-size:1.1rem}p{color:var(--sub);font-size:.875rem}button{margin-top:1.1rem;padding:.55rem 1.4rem;background:var(--btn);border:none;border-radius:8px;color:white;font-size:.9rem;font-weight:500;cursor:pointer}button:hover:not(:disabled){background:var(--btn-h)}button:disabled{opacity:.5;cursor:not-allowed}#msg{font-size:.8rem;color:var(--sub);margin-top:.65rem;min-height:1.1em}</style></head><body><div><h2>Server not running</h2><p>Start the Clocktopus server to continue.</p><button id="b" onclick="go()">Start Server</button><div id="msg"></div></div><script>async function go(){const b=document.getElementById('b'),m=document.getElementById('msg');b.disabled=true;b.textContent='Starting\u2026';await window.__TAURI__.core.invoke('start_server');m.textContent='Waiting for server\u2026';const t=setInterval(async()=>{try{if((await fetch('http://localhost:4001')).ok){clearInterval(t);location.href='http://localhost:4001'}}catch{}},1500)}</script></body></html>"#;
